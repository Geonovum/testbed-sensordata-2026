...
- **Idempotency is the client's problem, and it costs throughput.** Idempotency here means that
  writing the same reading twice still leaves exactly one copy on the server. Nothing in STA enforces
...
- **The same payload was accepted by one server and rejected by another.** We read the specification.
  ...
  (see section 4.7).
- **Well-formed STA data can still be hard to use.** The second server is entirely valid and carries
...
Section 4.10 then works through a question the findings raise repeatedly, because it determines what
anyone should actually do about them. When something goes wrong, is it the FROST-Server
implementation, the specific deployment, or the standard? The three have different remedies on
...
Our starting position shaped every decision that follows. The sensors in scope are fill-level sensors
...
That situation is common rather than special, and it is worth stating plainly, because it determines
what connecting a sensor to STA means in practice. In most existing sensor estates the sensor is
already connected to something. The integration problem is not device to server. It is platform to
...
The system has three deliberate seams.
...
WebSockets. Entity upserts always go over HTTP. We can write several targets in parallel, which is
what we call dual-write.

### 1.2 How a waste container becomes STA entities

This report says more than once that mapping to STA was the cheap part, so we show the mapping rather
than assert it. Table 3 is the whole of it. One container arrives from the vendor API as an id, a
...
| "Fill level"           | **ObservedProperty**  | `Fill level`                                    | **Shared by every container**, full stop                                                                               |
...
and it is also exactly what raced and duplicated when we resolved datastreams in parallel (see
section 4.4). The design decision and the defect are the same decision.
...
Each stored Observation carries two more things, and the standard gives us a home for both. We would
otherwise have had to invent somewhere to put them. `resultQuality.validated_by` and
...
Table 3 above is the destination. This section shows the journey for both kinds of source we support,
because we transform them very differently. A vendor REST API has a model with nothing to do with
STA. Another STA server already has the right model, and the job there is faithful copying.

The shapes below follow the source schemas and our mapping code exactly. The values illustrate rather
...
**What happens in that transformation, beyond renaming fields.** Six things took real thought, and
they represent what any vendor-to-STA adapter deals with.
...
2. **Coordinates swap.** REEN reports `latitude, longitude`. GeoJSON, and therefore STA, expects
   ...
   about it. This is the easiest thing to get wrong in the whole pipeline, and it fails in silence:
   valid JSON, valid GeoJSON, container in the North Sea.
...
4. **Forecasts have to go before the cursor sees them.** `/fillLevels` mixes settled estimates with
   ...
   measurement between now and that forecast date would then be dropped as `before_cursor`. One
   vendor quirk, weeks of silent data loss. The adapter discards anything newer than now.
5. **We honour the vendor's own quality flag.** REEN sets `confidence` to 100 for a normal reading,
   ...
   drops by reason instead of discarding them quietly.
...
1. **`result` is not a number.** `true` travels from source to destination untouched, as `ResultRaw`,
   and the validator skips its numeric range checks, because a boolean has no range. Our own model
   typed `result` as `float64`. The standard never did (see section 4.6).
2. **We keep the source's vocabulary, however imperfect.** The ObservedProperty is `motion` and its
   definition points at Wikipedia. We do not improve it, because a copy that quietly reclassifies its
   source is worse than one that copies faithfully. This is also exactly the material sections 4.8
...
3. **Validate.** Drop readings with a missing or non-numeric result, drop fill levels outside 0-100,
   ...
   count rejections by reason instead of discarding them quietly.
...
Steps 4 and 5 are the expensive ones, and they are expensive for the same reason. The standard gives a
client no way to say "create this if it does not already exist", so both steps read before they write.
...
received from it point the same way. That single fact upgraded this work from a single-server exercise
to a genuine interoperability test. Every difference between the two targets in section 4 is a
...
upsert and idempotency behaviour explicit and easy to inspect, and that is exactly where the subtle
problems in section 4 showed up. A library would have hidden them rather than prevented them. That is
...
Read that table as one sentence. Conforming to the standard was cheap, and production robustness
against a real network, a real authentication scheme and a real non-standard server is where the
engineering lived. That second cost exists whether or not you use a standard, which is exactly why
adopting one is close to free. Section 7.3 expands on this.
...
to look at your own configuration. We spent time on the former when the answer was the latter. It is a
small illustration of a theme that returns in section 6.4. When a server's error response does not
carry the right meaning, the client cannot route the problem to the right place, and whoever is
...
not cover, and their maintainer has said they would like to see them standardised. The problem is not
that they exist. It is that a client cannot discover that they do.
...
The concrete failure mode is worth spelling out, because it is worse than a naming clash. Our upsert
...
A prefix makes our entities unmistakably ours, and it does that by making them not match anyone
else's. So if two parties genuinely should converge on one shared `Fill level` concept, prefixing
guarantees they never will. We have traded accidental collision for guaranteed fragmentation. That is
...
The race produces quietly wrong data, and our existing conflict-refetch branch never fired. This is
...
**How we found it,** because this part generalises. Not from an error, because there was none. A
routine log line reported `sta duplicate entity ... count=5` when a later run's name lookup returned
...
That is the uncomfortable generalisation. **The only reason we detected a silent data-integrity bug
was a defensive check we wrote for unrelated reasons.** Any pipeline doing name-keyed upserts against
...
and reuse the lowest id. That is precisely why it is easy to miss. The symptom appears once, early,
and then goes quiet.
...
while MQTT publish kept up easily.
...
The shape of the problem is worth naming, because it is not a tuning issue. On the MQTT path the write
itself is asynchronous and fast, while the safety check in front of it is synchronous and slow. The
faster transport is therefore throttled by the slower protocol, and adding MQTT capacity does nothing
at all, because the queue forms in front of the probe. Under burst load the probe hit the client
timeout while publishes kept pace comfortably, which reads like a transport failure and is not one.
...
One case still needs our own bookkeeping. The invariant holds when a source only ever moves forward in
time. Ours does not always, because the vendor API mixes settled estimates with forecasts and can
re-issue a corrected value for a timestamp we have already passed. For a back-dated correction, later
than the newest stored reading is no help, so the write-log stays. See section 6.2.

It is worth being precise about who owns this one. The probe exists because the standard offers no
idempotent write, so this is neither a FROST defect nor a deployment defect. Both servers behave
correctly. In the framing of section 4.10 it is neither verdict A nor verdict B. The specification is
not ambiguous here, it is simply silent, and the cost of that silence is a round-trip per observation,
paid by every producer independently.
...
**Status: closed.** The change stayed contained. The ingest core, state store, dual-write and MQTT
paths needed no modification, which is the adapter seam paying off.
...
into the canonical layer. The standard is telling you where the variability is, and it is worth
listening.
...
**What the specification actually says.** We read the normative text, because it determines whose
...
state. That is a server-side defect, not a standoff between two defensible readings.
...
string-shaped content. That is an entirely understandable place to land, and it is still narrower than
...
column as a string. The declared type is `Any`, and the surrounding prose assumes a string. That mixed
signal is what makes this mistake easy to make, and it is why section 6.4 asks for the loose typing to
be pinned down rather than asking only for a bug fix.
...
This remains the most instructive interoperability finding in the testbed, and the lesson survives the
reattribution. **A server can present itself as STA-conformant and still reject a payload the
...
expensive, so this section works the attribution question through properly. We use the
`Sensor.metadata` rejection in section 4.7 as the worked example, then generalise.
...
payload, the disagreement itself is the evidence. Exactly one of them can be right, or the
specification is ambiguous enough that both are. With one server, or two builds of one server, you
...
There are three, not two, and teams miss the middle one.
...
The test is always the same, and it is not "which server is more popular" or "which one did we develop
against". **Read the normative text and check whether it mandates, permits, or is silent.**
...
FROST was being lax, the client had unknowingly built on that laxity, and the strict server was right.
Nobody's specification needs changing. The permissive server needs to stop being permissive, and every
...
as valid JSON. Our payload is legal. The rejection is a Laravel `string` validation rule, not a reading
of the standard (see section 4.7). The strict server is wrong here, which is the exact mirror image of
the previous case, on the same server.
...
permitted and the rejection is over-strict. But the specification genuinely does not say what to do
about a partially null unit, which is what that stream had. Part B, part C.

The same server is wrong in one direction on one field and right in the other direction on another.
That is not a criticism of it. It is what independent implementation looks like before a test suite
exists.

#### 4.10.4 The positive and negative case for each diagnosis

Taking our own `metadata` finding, and arguing both sides honestly:

**Treating it as an implementation bug.** In favour: it is true, and it is fixable now, with one
validation rule, one deployment and no committee. It keeps `Any` genuinely flexible, so the legitimate
...
**Treating it as a specification problem.** In favour: fixing the text fixes every implementation at
once and permanently, and it makes the requirement testable, which is what turns interoperable in
principle into interoperable in practice. The ambiguity is real and demonstrable: `Any` as the declared
type, three enumerated encodingTypes that are all string-shaped, and a sentence telling clients to do
string parsing. That is genuinely mixed signalling. Against: timescales run to years, so it helps
...
an easy place to park a problem so that nobody owns it. It reads as progress and produces none.

**And the option nobody argues for but everybody takes: treating it as your own problem.** Just
serialise the object to a string and move on. In favour: it works this afternoon, on every server.
Against: it is how an unstated constraint becomes the de facto standard. If every client degrades to
the strictest server it meets, the strict behaviour spreads, the loose but legal form dies out, and the
specification's actual permission becomes untestable because nothing exercises it any more. This is
the failure mode we care most about, because it is invisible, and because it is what we ourselves were
about to do.
...
2. **Work around it per target, never globally.** Our serialisation is a per-`Target` setting precisely
   so the strict server's constraint does not quietly become our client's behaviour everywhere. That is
   the difference between accommodating one server and capitulating to it.
...
4. **Push hardest on the thing that fixes the whole class,** a published interoperability test suite
   ...
   reject would have caught every case in this section: the lax server, the strict server and the
...
server stop complaining. In doing so they quietly ship someone else's misreading as their own
behaviour, and, worse, remove the evidence that anything was ever wrong.
...
We mapped a domain the standard's authors plainly did not have in mind, municipal waste containers,
...
telemetry, seismics and weather onto the same seven entities without touching the mapping layer. A
model that absorbs both without strain is a well-designed model.
...
- `$filter=phenomenonTime gt <cursor>` with `$orderby` and `$top` is an incremental-sync primitive we
  got for free. It is exactly the query a cursor-based reader needs, and we did not have to negotiate
  it with anyone;
...
call, and no risk of an orphaned entity between two writes. A small feature, and a materially simpler
write path.
...
writes from HTTP POST to MQTT publish over WebSockets with no change to the mapping layer at all: same
payload, different pipe. Changing transport characteristics without touching your data model is unusual
...
This is what turns STA from a data format into an integration strategy. Section 7 is essentially an
elaboration of this paragraph.
...
Copying one STA server into another is therefore a small program, not a project. Federation, mirroring,
aggregation and archival all follow from this for free.
...
deliver. They are not the ones that took us longest. Each item states the concrete problem we hit and a
...
result less useful after you have implemented it perfectly. We achieved interoperability of form, and
not of meaning, and meaning is what a data consumer actually wants.
...
could specify, because it is exactly the layer the base standard deliberately leaves open, and exactly
the layer where gemeenten, waterschappen and their suppliers gain from agreeing together.
...
`GET ?$filter=name eq ...`, then POST. That is an extra round-trip per entity, and it is racy. We hit
exactly this and produced five copies of two entities (see section 4.4).
...
That matters for the ask, and it matters in the domain of the very server we write to, since WBD is a
...
it is right not to, because STA does not require them to be unique. So there is no 409, no error and no
signal, just quietly duplicated data. The standard makes the naive implementation the incorrect one
...
and the primary key does the deduplication that no amount of client-side locking can guarantee. It
removes the race and a round-trip per entity in one move.
...
1. Make client-specified ids **discoverable**. The v2 text says a service *may* allow them, which is
   exactly the kind of optional behaviour a portable client currently has to establish by trial. A
...
duplication into a clear 409 on the very first run.

It also reframes the finding. On a server that several parties write into, which constraints exist is a
deployment decision that every writer depends on and none of them can see. That is not a gap in the
specification. It belongs in a national profile, alongside the vocabulary work in section 6.1, and it
is a second argument for the capabilities document in section 6.3, because an administrator's added
constraints are precisely the kind of local truth a client cannot guess.
...
OGC API - Features solved exactly this with a required `/conformance` endpoint. It is also the cheapest
...
string and rejected a payload the specification allows. **The defect was the server's, and the
ambiguity that invited it is the standard's,** and this item asks only for the second half.
...
**Why it matters.** A field declared `Any`, with prose pulling the other way, invites divergent
implementations. Divergence in the core entity set is what breaks the assumption most adopters start
with, that STA-conformant implies portable. Without a classifiable error model, each divergence becomes
...
capability gap. On the evidence it is not: Hylke van der Schaaf uses data arrays and batch processing
as his normal import path in
[discussion #22](https://github.com/Geonovum/testbed-sensordata-2026/discussions/22), so the facility
works where it matters. The real problem is the same one as everywhere else in this section, that a
client cannot discover whether a given server offers it (see section 6.3).
...
consumers, so you cannot negotiate with them in advance. A standard is how you serve consumers you have
not met.
...
hard because ingestion is hard, not because STA is hard. The STA-specific part was the cheapest line in
the table.

So the incremental cost of conforming is small, and the interoperability upside is disproportionate.
Bespoke is not cheaper. It is the same price with none of the benefit.
...
calculation is not "the standard versus our simple REST API". It is "deploy an existing server versus
design, build, document, version and support an API forever".
...
system, cadence, provenance and synthetic markers, with no loss and no cost to conformance. The common
objection, that the standard does not model our domain, is largely answered by this. Model the bulk of
it in standard entities and carry the rest in `properties`. You lose nothing, and the bulk becomes
interoperable immediately.
...
afternoon from the specification is a standard with genuinely low adoption risk, and that is not true of
...
Test against more than one server implementation before you declare victory. We found a hard
...
so the server, and not our client, was at fault. Conformance claims are not self-verifying. Expect to
build idempotency yourself as well (see section 6.2).

Neither of these changes the conclusion. Both change your project plan, and it is better to know on day
one.
...
1. **Specify a Dutch STA profile that closes the semantic gap** (see section 6.1): a mandated
   ...
   This is the highest value per unit of effort of anything on the list. It is precisely the layer the
...
2. **Add namespacing and constraint guidance for shared, multi-party servers** (see sections 4.3 and
   6.2): entity-name conventions, an ownership convention in `properties`, and, importantly, which
